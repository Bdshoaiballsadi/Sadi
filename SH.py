...
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrakingxxx-cov', 'r').read()
kex=(f"https://github.com/Bdshoaiballsadi/new/blob/main/try.txt")
...
try:
    clear()
    print("\033[1;33m Note: This script cannot be run without my permission,exit")
    linex()
    ...
    if "Version-1.5" in response:
        if kex in response:
            ...
            print("\033[1;36m Your key: "+fkeyx);subprocess.check_output(["am", "start", "https://api.whatsapp.com/send?phone=+8801617383560&text="+(" Hi SHOAIB sir i want Buy Auto Create Ids Tool Please Approve My Token\n Token:- "+ fkeyx)]);time.sleep(2)
...
def Create():
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write('\r\r\033[1;37m [SHOAIB-CREATE] \033[1;32mOK:%s \033[1;37m'%(ok));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
           ...
           if "9":
              ...
              if requests=="chk":
                ...
                pass
                 ...
                 print("\r\r\033[1;32m [SHOAIB-OK] "+uid+'|'+pas+'|'+coki)
                 ok+=1
           ...
           pass
    file="/sdcard/SHOAIB/CREATE-OK.txt"
    u=5000
    clear()
    print(' Use Airplane Or Use IP Changer if ids coming cp ')
    print(' ids will be save in /sdcard/SHOAIB/CREATE-OK.txt')
    linex()
